...
def simBus(bus, numStops = 6, loopLen = 1200, meanArrival = 90,
    ...
    while time < simTime:
        ...
        for i in range(bus.getSpeed()):
            busLoc += 1 #for each busspeed movment, move forward bus location
            if (busLoc)%(loopLen/numStops) == 0: #if location is found, stop bus
                break
        #see if there is a passenger waiting to enter queue
        if lastArrival + nextJob.interArrival() <= time: #if passenger made it in time
            #passengers arrive simultaneously at each stop
            for stop in stops: #are jobqeues (busstops [using FIFO by default])
                stop.arrive(nextJob) #how strange eg...passsengers arraive at each stop and enter each queue
            nextJob.queue(time) #time they are in queue
            lastArrival = time #assining last arrival time
            nextJob = Passenger(meanArrival, meanWork) #get the new job (passenger)
        ...
        if (busLoc)%(loopLen/numStops) == 0:
            ...
            while stops[nextStop%numStops].length() > 0:
                try:
                    bus.enter() #passenger enters bus
                except:
                    break
                p = stops[nextStop%numStops].depart() #passenger leaves queue (cold fucking street)
                totWait += time - p.queuedTime() #total wait time
                totPassengers += 1 #total passengers on bus
                time += p.work() #advance time, but not bus because bus isn't moving but passengers take time to get on
            try:
